...
class InvestigationController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            // Pre-resolve Laboratory service IDs to avoid nested EXISTS
            $labCategoryIds = DB::table('service_categories')
                ->where('name', 'Laboratory')->pluck('id');
            $labServiceIds  = MedicalService::whereIn('service_category_id', $labCategoryIds)->pluck('id');
            ...
            $query = Investigation::with(['patient', 'doctor', 'medicalService.serviceCategory'])
                ->whereIn('medical_service_id', $labServiceIds)
            ...
            if ($request->filled('service_category')) {
                $catIds = MedicalService::where('service_category_id', $request->service_category)->pluck('id');
                $query->whereIn('medical_service_id', $catIds);
            }
        ...
        return view('investigations.index', compact('doctors', 'serviceCategories', 'dateFrom', 'dateTo'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            //Get visit category id
                if($request->patient_category_id) {
                    ...
                    return response()->json([
                        'success' => false,
                        'message' => 'Visit category ID is required for pricing'
                    ], 400);
                }

            // Get medical service information, with pricing according to visit category id
            $service = MedicalService::findOrFail($request->medical_service_id);

            //Get pricing for the service and visit category
            $pricing = $service->pricing($visitCategoryId);
            ...
            if ($pricing) {
                $cashAmount = (float) ($pricing['cash_amount']);
                $insuranceCoveredAmount = (float) ($pricing['insurance_covered_amount']);
            }
            // Return error if no pricing found for the service
            else {
                return response()->json([
                    'success' => false,
                    'message' => 'No pricing available for the selected medical service'
                ], 400);
            }
            ...
            $investigation = Investigation::create([
                ...
                'quantity' => $quantity,
                'cash_amount' => $cashAmount * $quantity,
            ...
            // Save form data to investigation_form_data if a clinical form was filled
            if ($request->filled('clinical_data')) {
                $formData = json_decode($request->clinical_data, true);
                if (is_array($formData) && count($formData)) {
                    InvestigationFormData::create([
            ...
            if (request()->ajax()) {
                return response()->json([
                    ...
                    'data' => [
                        ...
                        'price' => $investigation->cash_amount,
        ...
        try {
            ...
            foreach ($request->services as $serviceData) {
                ...
                if ($service->currentPricing && $service->currentPricing->count() > 0) {
                    ...
                    $cashAmount = (float) ($pricing->cash_price ?? $pricing->price ?? 0.00);
                    $insuranceCoveredAmount = (float) ($pricing->insurance_price ?? 0.00);
                }
                // Fallback to service cost if no pricing
                elseif (isset($service->cost) && $service->cost > 0) {
                    $cashAmount = (float) $service->cost;
                }
