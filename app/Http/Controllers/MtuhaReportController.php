...
class MtuhaReportController extends Controller
{
    ...
    public function select(Request $request)
    {
        ...
        return view('reports.mtuha_select', compact('year', 'month'));
    }
    ...
    $data['month_name'] = $this->service->monthName($month);
        ...
        // PDF export — uses a standalone document (no app layout/nav) so the
        // PDF contains only the report table, not the sidebar/header chrome.
        if ($request->has('pdf') && (int) $request->input('pdf') === 1) {
            $html = view('reports.pdfs.mtuha_month', $data)->render();
