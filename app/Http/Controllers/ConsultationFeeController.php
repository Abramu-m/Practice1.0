...
class ConsultationFeeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($search = $request->input('search')) {
            $query->where(function ($q) use ($search) {
                $q->where('cash_amount', 'like', "%{$search}%")
                  ->orWhere('covered_amount', 'like', "%{$search}%")
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'cash_amount' => 'required|numeric|min:0|max:999999.99',
            'covered_amount' => 'required|numeric|min:0|max:999999.99',
            ...
            'cash_amount.numeric' => 'Cash amount must be a valid number.',
            'cash_amount.min' => 'Cash amount cannot be negative.',
            'cash_amount.max' => 'Cash amount cannot exceed 999,999.99.',
            'covered_amount.required' => 'Covered amount is required.',
            'covered_amount.numeric' => 'Covered amount must be a valid number.',
            'covered_amount.min' => 'Covered amount cannot be negative.',
            'covered_amount.max' => 'Covered amount cannot exceed 999,999.99.'
        ]);
