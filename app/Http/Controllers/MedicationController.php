...
class MedicationController extends Controller
{
    ...
    public function create()
    {
        ...
        $drugClassesByCategory = DrugClass::where('is_active', true)
            ->orderBy('category')
            ->orderBy('name')
            ->get(['id', 'name', 'category'])
            ->groupBy('category');
        
        return view('medications.create', compact('categories', 'formulations', 'dispensingUnits', 'drugClassesByCategory'));
    }
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'msd_code_id' => 'nullable|exists:msd_codes,id',
            ...
            'is_active' => 'boolean',
            'drug_classes' => 'nullable|array',
            'drug_classes.*' => 'integer|exists:drug_classes,id',
        ]);

        try {
            DB::beginTransaction();

            $drugClassIds = $validated['drug_classes'] ?? [];
            unset($validated['drug_classes']);
            
            // Set default stock quantity to 0 since it will be managed through GRN
            $validated['stock_quantity'] = 0;
            
            $medication = Medication::create($validated);
            $medication->drugClasses()->sync($drugClassIds);
    ...
    {
        $medication->load(['storeCategory', 'formulation', 'dispensingUnit', 'prescriptions', 'ledgerEntries', 'drugClasses']);
    ...
    public function edit(Medication $medication)
    {
        ...
        return view('medications.edit', compact('medication', 'categories', 'formulations', 'dispensingUnits', 'drugClassesByCategory'));
    }
    ...
    {
        $medications = Medication::with('storeCategory')
            ->orderBy('generic_name')
            ->get(['id', 'generic_name', 'brand_name', 'strength', 'category_id', 'selling_price', 'discount_percentage', 'is_active']);

        return view('medications.pricing', compact('medications'));
    }
    ...
    public function updatePrice(Request $request, Medication $medication)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Price updated for ' . $medication->generic_name,
        ]);
    }

    public function tracerIndex()
    {
        $medications = Medication::with('storeCategory')
            ->where('is_active', true)
            ->orderBy('generic_name')
            ->get(['id', 'generic_name', 'brand_name', 'strength', 'category_id', 'stock_quantity', 'is_tracer']);

        return view('medications.tracer', compact('medications'));
    }

    public function toggleTracer(Medication $medication)
    {
        $medication->update(['is_tracer' => !$medication->is_tracer]);

        return response()->json([
            'success'   => true,
            'is_tracer' => $medication->is_tracer,
            'message'   => $medication->is_tracer
                ? "{$medication->generic_name} added to tracer medicines"
                : "{$medication->generic_name} removed from tracer medicines",
        ]);
    }
}
