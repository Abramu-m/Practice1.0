...
class ProcedureConsumption extends Model
{
    ...
    {
        return sprintf(
            '%s consumed %s %s of %s for %s procedure',
            $this->consumedBy->name ?? 'Unknown User',
            $this->quantity_used,
            $this->item_unit,
            $this->item_name,
            $this->procedure->name ?? 'Unknown Procedure'
        );
    }
     ...
     */
    public static function createFromLocationStock($locationStock, $procedure, $patient, $quantity, $consumedBy, $notes = null)
    {
        $data = [
    ...
    public static function getConsumptionStats($startDate, $endDate, $departmentId = null)
    {
        ...
        return [
            ...
            'unique_procedures' => $query->distinct('procedure_id')->count(),
        ];
    }
    ...
    /**
     * Get procedure consumption report
     */
    public static function getProcedureReport($procedureId, $startDate, $endDate)
    {
    ...
    /**
     * Get procedure consumption trends
     */
    public static function getProcedureTrends($procedureId, $months = 6)
    {
        $startDate = now()->subMonths($months)->startOfMonth();
        
        return self::where('procedure_id', $procedureId)
                  ->where('consumed_at', '>=', $startDate)
                  ->selectRaw('
                      YEAR(consumed_at) as year,
                      MONTH(consumed_at) as month,
                      COUNT(*) as consumption_count,
                      SUM(quantity_used) as total_quantity
                  ')
                  ->groupByRaw('YEAR(consumed_at), MONTH(consumed_at)')
                  ->orderByRaw('YEAR(consumed_at), MONTH(consumed_at)')
                  ->get();
    }

    /**
     * Get department procedure consumption
     */
    public static function getDepartmentProcedureConsumption($departmentId, $startDate, $endDate)
    {
        return self::with(['procedure', 'medication', 'consumable'])
                  ->where('department_id', $departmentId)
                  ->whereBetween('consumed_at', [$startDate, $endDate])
                  ->get()
                  ->groupBy('procedure_id')
                  ->map(function ($items, $procedureId) {
                      $procedure = $items->first()->procedure;
                      return [
                          'procedure_name' => $procedure ? $procedure->name : 'Unknown',
                          'total_consumptions' => $items->count(),
                          'total_quantity' => $items->sum('quantity_used'),
                          'total_cost' => $items->sum('consumption_cost'),
                          'unique_items' => $items->pluck('medication_id')->merge($items->pluck('consumable_id'))->filter()->unique()->count(),
                          'consumptions' => $items
                      ];
                  });
    }
