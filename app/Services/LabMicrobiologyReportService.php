...
{
    /**
     * Build microbiology lab report
     */
    public function buildReport(): array
    {
        $investigations = $this->getMicrobiologyInvestigations();
    ...
    /**
     * Get microbiology investigations
     */
    private function getMicrobiologyInvestigations()
    {
        return Investigation::join('patient_visits', 'investigations.visit_id', '=', 'patient_visits.id')
            ...
            ->where(function ($query) {
                $query->where('service_categories.name', 'LIKE', '%Microbiology%')
                    ->orWhere('medical_services.name', 'LIKE', '%Culture%')
                    ->orWhere('medical_services.name', 'LIKE', '%Sensitivity%');
            })
