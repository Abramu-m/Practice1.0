...
class FinancialTransactionService
{
    ...
    /**
     * Create transaction from general expense
     */
    public function createFromGeneralExpense(GeneralExpense $expense): ?FinancialTransaction
    {
        try {
            if ($expense->status !== 'approved' || $expense->amount <= 0) {
                return null;
            }

            // Check if transaction already exists for this expense
            $existingTransaction = FinancialTransaction::where('source_type', 'general_expense')
                ->where('source_id', $expense->id)
                ->first();

            if ($existingTransaction) {
                Log::info("Transaction already exists for expense {$expense->id}");
                return $existingTransaction;
            }
            ...
            $transaction = FinancialTransaction::create([
                ...
                'subcategory' => $expense->expense_category,
                ...
                'patient_paid_amount' => $expense->amount,
                'insurance_covered_amount' => 0,
                'status' => 'completed',
            ...
            Log::info("Created financial transaction {$transaction->transaction_number} for expense {$expense->id}");
            
            return $transaction;
            
        } catch (\Exception $e) {
            Log::error("Failed to create transaction for expense {$expense->id}: " . $e->getMessage());
            return null;
        }
    }
