...
class ReceiptGenerationService
{
    ...
    protected function getClinicInfo()
    {
        return [
            ...
            'address' => config('app.clinic_address', 'Clinic Address'),
            'phone' => config('app.clinic_phone', 'Phone Number'),
            'email' => config('app.clinic_email', 'clinic@example.com'),
            'license' => config('app.clinic_license', 'License Number'),
            'tax_id' => config('app.clinic_tax_id', 'Tax ID')
        ];
    }
    ...
    public function emailReceipt(FinancialTransaction $transaction, $recipientEmail = null)
    {
        ...
        $email = $recipientEmail ?? $transaction->patient->email ?? null;
        
        if (!$email) {
            throw new \Exception('No email address available for patient');
        }
