...
$stats = [
    ...
    'medication_policies' => App\Models\CdsDosageLimit::count(),
...
$dosageLimits = App\Models\CdsDosageLimit::with('medication')->where('is_active', true)->get();
foreach ($dosageLimits as $limit) {
    $name = optional($limit->medication)->name ?? '(no medication)';
    echo sprintf("• %-25s max single: %s  max daily: %s\n",
        $name,
        $limit->max_single_dose_adults ?? '—',
        $limit->max_daily_dose_adults ?? '—'
    );
    echo "\n";
}
