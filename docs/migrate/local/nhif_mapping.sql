...
-- Prerequisite: pricing_local.sql must have run first — it sets the
-- NHIF category's tariffs_table = 'nhif_tariffs', which the INNER JOIN
-- below relies on to resolve the patient_category_id.
-- ============================================================

-- Medical services -> medical_service_insurance_map
INSERT INTO practice.medical_service_insurance_map
...
FROM medcom1_0.nhif_service_mapping sm
INNER JOIN practice.medical_services ms ON ms.id = sm.local_svsvid
INNER JOIN practice.patient_categories pc ON pc.tariffs_table = 'nhif_tariffs'
...
INSERT INTO practice.medication_insurance_map
    (medication_id, patient_category_id, insurance_item_code, created_at, updated_at)
SELECT m.id, pc.id, pm.nhif_itemcode, NOW(), NOW()
FROM medcom1_0.nhif_pharmacy_mapping pm
INNER JOIN practice.medications m ON m.id = pm.local_did
INNER JOIN practice.patient_categories pc ON pc.tariffs_table = 'nhif_tariffs'
