...
Route::get('/investigation-form/{formType}', function($formType) {
    try {
        ...
        if ($visitId && !$consultationId) {
            // Lab-only visit: load patient data directly from PatientVisit
            $patientVisit = \App\Models\PatientVisit::with(['patientInfo', 'doctorInfo'])
                ->findOrFail($visitId);

            $patient = $patientVisit->patientInfo;
            $doctor  = $patientVisit->doctorInfo;
            ...
            $visit = (object) [
                'id'         => $patientVisit->id,
                'date'       => $patientVisit->created_at->format('Y-m-d'),
                'time'       => $patientVisit->created_at->format('H:i'),
                'created_at' => $patientVisit->created_at,
                ...
                'laboratory' => (object) [
                    'serial_number' => 'LAB-' . date('Y') . '-' . str_pad($patientVisit->id, 4, '0', STR_PAD_LEFT),
                    'technician'    => '',
                    'reviewer'      => '',
                ],
            ];
...
Route::get('/result-template/{templateName}', function($templateName) {
    try {
        ...
        // Get the investigation with patient, consultation, and ordering form data
        $investigation = \App\Models\Investigation::with([
            'patient',
        ...
        $templateMap = [
            // TB — result entry uses genxpert_tb (tb.blade.php moved to lab/forms/)
            'tb'                 => 'lab.result_templates.genxpert_tb',
            'genxpert_tb'        => 'lab.result_templates.genxpert_tb',
            'zn_stain_tb'        => 'lab.result_templates.zn_stain_tb',
            // CD4
            ...
            'pbs_rbc_morphology' => 'lab.result_templates.pbs_rbc_morphology',
            'psa_semiquantitative' => 'lab.result_templates.psa_semiquantitative',
            'gram_stain'         => 'lab.result_templates.gram_stain',
            'mrdt_malaria'       => 'lab.result_templates.mrdt_malaria',
            'multistix'          => 'lab.result_templates.multistix',
            'wet_prep_microscopy' => 'lab.result_templates.wet_prep_microscopy',
            'stool_analysis'      => 'lab.result_templates.stool_analysis',
            'spermiogram'         => 'lab.result_templates.spermiogram',
            'sterility_anamnesis' => 'lab.result_templates.sterility_anamnesis',
        ];
