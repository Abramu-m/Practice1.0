 ...
 * 4. Call openLabModal(@json($visit)) or openLabModal(json_encode($visit)) to open the modal
 * The visit should always be (sent from the Controller) be with the necessary relationships loaded (patient_info, consultation, visit_category) so that the modal can extract all needed context without requiring additional parameters.
 * 5. Extract from the visit array (using the pattern $variable = visit.consultation.id;) the necessary context (visit ID, patient ID, consultation ID if applicable)
 * No more context, the process is streamlined to just require the visit object, and the backend will handle the rest.
 */
 ...
 * @param {Object} visit - The visit object containing necessary context (visitId, patientId, consultationId, patientName)
 */
window.openLabModal = function openLabModal(visit) {
    // Handle different parameter combinations
    let actualVisitId, consultationId, patientName, patientId, visitCategoryId;
    ...
    if (visit && typeof visit === 'object') {
        ...
        console.error('Invalid visit parameter passed to openLabModal:', visit);
        return;
    }
   
    
    // Store parameters in a global variable for later use
    
    // Set patient and visit information
    $('#modal_patient_id').val(patientId);
    $('#modal_visit_id').val(actualVisitId);
    $('#modal_consultation_id').val(consultationId || '');
    $('#modal_patient_name').text(patientName);
    $('#modal_patient_category_id').val(visitCategoryId);
     
    
    // Reset form
    resetInvestigationForm();
    
    // Load existing investigations (pass actualVisitId)
    loadExistingInvestigations(actualVisitId);
...
function loadExistingInvestigations(visitId) {
    ...
    // URL 
    const url = `/patient-visits/${visitId}/investigations-partial`;
...
function searchModalMedicalServices(query) {
    const visitCategoryId = $('#modal_patient_category_id').val();
    ...
    $.ajax({
        ...
        data: { 
            query: query, 
            limit: 10,
            patient_category_id: visitCategoryId
        },
        success: function(response) {
...
function showModalServiceSuggestions(services) {
    ...
    services.forEach(function(service) {
        const cash = service.cash || 0;
        const covered = service.covered || 0;
        const categoryName = service.category || 'General';
        const hasPricing = service.has_pricing || false;
        const formattedCash = parseFloat(cash).toLocaleString('sw-TZ', {style: 'currency', currency: 'TZS'});
        const formattedCovered = parseFloat(covered).toLocaleString('sw-TZ', {style: 'currency', currency: 'TZS'});
        ...
        html += `
            <div class="service-suggestion-item p-3 border-bottom bg-white" 
                 ...
                 data-service-cash="${cash}"
                 data-service-covered="${covered}"
                 ...
                 <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="text-end">
                        <strong class="text-primary"> ${formattedCash}</strong>
                        <br>
                        <small class="text-success">Covered: ${formattedCovered}</small>
    ...
    container.find('.service-suggestion-item').on('click', function() {
        ...
        const cash = $(this).data('service-cash');
        const covered = $(this).data('service-covered');
        ...
        // Store metadata
        $('#modal_service_search').data('cash', cash);
        $('#modal_service_search').data('covered', covered);
        ...
        showModalServiceInfo(serviceId, serviceName, cash, covered, serviceCategory, hasPricing, requiresForm, formType);
 ...
 */
function showModalServiceInfo(serviceId, serviceName, Cash, Covered, serviceCategory, hasPricing = false, requiresForm = false, formType = null) {
    const info = $('#modal-service-info');
    const quantity = parseInt($('#modal_investigation_quantity').val()) || 1;
    const cash = (parseFloat(Cash) * quantity).toFixed(2);
    const covered = (parseFloat(Covered) * quantity).toFixed(2);
    ...
    let html = `
        <div class="row">
            ...
            <div class="col-md-4 text-end">
                <strong>Cash Price: ${parseFloat(Cash).toLocaleString('sw-TZ', {style: 'currency', currency: 'TZS'})}</strong><br>
                <strong>Covered Price: ${parseFloat(Covered).toLocaleString('sw-TZ', {style: 'currency', currency: 'TZS'})}</strong><br>
            </div>
        </div>
    `;
...
function loadFormDisplay(formType) {
    ...
    const params = {};
    const consultationId = $('#modal_consultation_id').val();
    const visitId        = $('#modal_visit_id').val();
...
function saveLabInvestigation() {
    ...
    // URL only to investigations endpoint
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                // If there's a loadInvestigations function, call it
                if (typeof window.loadInvestigations === 'function') {
                    window.loadInvestigations();
                }
