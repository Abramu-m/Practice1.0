...
@section('patient_info')
    <div class="d-flex align-items-center">
        <div>
            <h4 class="mb-0">{{ $visit->patientInfo->first_name }} {{ $visit->patientInfo->middle_name ?? '' }} {{ $visit->patientInfo->last_name }}
                <small class="text-muted">
                    MRN: <span class="badge bg-info">{{ $visit->patientInfo->mr_number }}</span> | 
                    Age: <strong>{{ $visit->patientInfo->age ?? 'N/A' }}</strong> years |   
                    Gender: <strong>{{ $visit->patientInfo->gender ?? 'N/A' }}</strong>
                    Visit Category: <span class="badge bg-secondary">{{ $visit->visitCategory->description }}</span>
                </small>
            </h4>
        </div>
    </div>  
        ...
        <div id="home" class="tab-pane fade">
            ...
            <ul class="nav nav-tabs mb-3" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link d-flex flex-column justify-content-end align-items-center active" id="basic-info-tab" data-bs-toggle="tab" data-bs-target="#basic-info" type="button" role="tab">
                        <i class="fas fa-id-card mb-1"></i> Basic Information & Contact
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link d-flex flex-column justify-content-end align-items-center" id="visit-info-tab" data-bs-toggle="tab" data-bs-target="#visit-info" type="button" role="tab">
                        <i class="fas fa-calendar-check mb-1"></i> Current Visit
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link d-flex flex-column justify-content-end align-items-center" id="medical-alerts-tab" data-bs-toggle="tab" data-bs-target="#medical-alerts" type="button" role="tab">
                        <i class="fas fa-exclamation-triangle mb-1"></i> Medical History & Alerts
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link d-flex flex-column justify-content-end align-items-center" id="statistics-tab" data-bs-toggle="tab" data-bs-target="#statistics" type="button" role="tab">
                        <i class="fas fa-chart-bar mb-1"></i> Statistics
                    </button>
                </li>
            </ul>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($visit->consultation_fee)
                                            <strong class="text-success">Tsh {{ number_format($visit->consultation_fee, 2) }}</strong>
                                        @else
                            ...
                            @if(isset($testResults) && $testResults->count() > 0)
                                @php
                                    ...
                                    foreach ($testResults as $result) {
                                        ...
                                        $result->_tplCode = $tc;
                                        if (in_array($tc, ['legacy', 'narrative_lab', 'urinalysis', 'full_blood_picture', 'blood_count', 'genxpert_tb', 'zn_stain_tb', 'blood_grouping', 'pbs_microfilaria', 'pbs_malaria', 'pbs_rbc_morphology', 'psa_semiquantitative', 'gram_stain', 'cd4', 'general'])) $complexResults->push($result);
...
<!-- Case Summary Modal -->
<div class="modal fade" id="caseSummaryModal" tabindex="-1" aria-labelledby="caseSummaryModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="caseSummaryModalLabel">
                    <i class="fas fa-file-alt"></i> Case Summary Preview
                </h5>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="openCaseSummaryPdf()">
                    <i class="fas fa-print"></i> Open PDF
                </button>
            </div>
        </div>
    </div>
...
@include('consultations.partials.referral_modal')
...
<script>
    ...
    function printCaseSummary() {
        showModal('#caseSummaryModal');
    }

    function openCaseSummaryPdf() {
        window.open('{{ route('consultations.case_summary_pdf', $consultation->id) }}', '_blank');
    }
