{{-- ZN Stain Microscopy (AFB) Result Template --}}
@php
$params = [
    [
        'name'    => 'AFB Result',
        'options' => ['Negative (0 AFB/100 fields)', 'Scanty (1–9 AFB/100 fields)', '1+ (10–99 AFB/100 fields)', '2+ (1–10 AFB/field)', '3+ (>10 AFB/field)', 'Invalid'],
        'normal'  => ['Negative (0 AFB/100 fields)'],
        'critical'=> ['3+ (>10 AFB/field)'],
    ],
    [
        'name'    => 'Specimen Quality',
        'options' => ['Adequate', 'Inadequate', 'Saliva only'],
        'normal'  => ['Adequate'],
        'critical'=> [],
    ],
];
...
<div class="result-template-container" style="background-color:#fff;padding:15px;border-radius:5px;">
    <div class="text-center mb-3">
        <h6 class="text-primary"><i class="fas fa-microscope"></i> ZN Stain Microscopy (AFB)</h6>
        <small class="text-muted">Ziehl-Neelsen stain for Acid-Fast Bacilli — TB diagnosis</small>
    </div>
                ...
                <tr>
                    ...
                    <td>
                        <select class="form-select form-select-sm" name="parameters[{{ $idx }}][value]"
                                ...
                                onchange="multiOptStatus(this, 'zn')">
                    ...
                    <td>
                        <select class="form-select form-select-sm" id="zn-status-{{ $idx }}" style="background:#f0f0f0;pointer-events:none;cursor:not-allowed;appearance:none;" tabindex="-1" name="parameters[{{ $idx }}][status]">
...
if (typeof multiOptStatus === 'undefined') {
    function multiOptStatus(select, prefix) {
        ...
        if (!sel || !val) return;
        if (criticals.length && criticals.includes(val)) sel.value = 'critical';
