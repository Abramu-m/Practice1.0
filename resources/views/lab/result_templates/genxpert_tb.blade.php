{{-- GeneXpert MTB/RIF Result Template --}}
@php
$params = [
    [
        'name'    => 'MTB Detection',
        'options' => ['Not Detected', 'Detected – Very Low', 'Detected – Low', 'Detected – Medium', 'Detected – High', 'Invalid', 'Error', 'No Result'],
        'normal'  => ['Not Detected'],
        'critical'=> ['Detected – High'],
    ],
    [
        'name'    => 'RIF Resistance',
        'options' => ['Resistance NOT Detected', 'Resistance Detected', 'Indeterminate', 'Not Applicable'],
        'normal'  => ['Resistance NOT Detected', 'Not Applicable'],
        'critical'=> ['Resistance Detected'],
    ],
];
$existingParams = [];
...
<div class="result-template-container" style="background-color:#fff;padding:15px;border-radius:5px;">
    <div class="text-center mb-3">
        <h6 class="text-primary"><i class="fas fa-bacterium"></i> GeneXpert MTB/RIF</h6>
        <small class="text-muted">Molecular detection of Mycobacterium tuberculosis and rifampicin resistance</small>
    </div>
        ...
        <table class="table table-bordered table-sm">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <select class="form-select form-select-sm gx-select" name="parameters[{{ $idx }}][value]"
                                data-idx="{{ $idx }}"
                                data-normal="{{ implode('|', $param['normal']) }}"
                                data-critical="{{ implode('|', $param['critical']) }}"
                                onchange="gxUpdateStatus(this)">
                    ...
                    <td>
                        <select class="form-select form-select-sm" id="gx-status-{{ $idx }}" style="background:#f0f0f0;pointer-events:none;cursor:not-allowed;appearance:none;" tabindex="-1" name="parameters[{{ $idx }}][status]">
...
<script>
function gxUpdateStatus(select) {
    const idx      = select.dataset.idx;
    const normals  = (select.dataset.normal  || '').split('|');
    const criticals= (select.dataset.critical || '').split('|');
    const val      = select.value;
    const sel      = document.getElementById('gx-status-' + idx);
    if (!sel || !val) return;
