    ...
    <div class="receipt-container">
        ...
        <table class="service-table">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $item['name'] }}</td>
                    <td>{{ $item['quantity'] }}</td>
                    <td class="amount">Tsh {{ number_format($item['unit_price'], 2) }}</td>
                    <td class="amount">Tsh {{ number_format($item['total_price'], 2) }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div class="totals">
            <div class="total-row">
                ...
                <span class="total-amount">Tsh {{ number_format($payment_details['total_amount'], 2) }}</span>
            </div>
            @if($payment_details['insurance_covered'] > 0)
            <div class="total-row">
                <span class="total-label">Insurance Coverage:</span>
                <span class="total-amount">-Tsh {{ number_format($payment_details['insurance_covered'], 2) }}</span>
            </div>
            @endif
            <div class="total-row grand-total">
                <span class="total-label">Amount Paid:</span>
                <span class="total-amount">Tsh {{ number_format($payment_details['patient_paid'], 2) }}</span>
            </div>
            @if($payment_details['balance_due'] > 0)
            <div class="total-row">
                <span class="total-label">Balance Due:</span>
                <span class="total-amount">Tsh {{ number_format($payment_details['balance_due'], 2) }}</span>
            </div>
            @endif
        </div>
