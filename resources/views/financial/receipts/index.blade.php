...
$(document).ready(function() {
    // Initialize Select2 on patient dropdowns
    $('#patient_id').select2({
        placeholder: 'Select Patient',
        allowClear: true,
        width: '100%',
        dropdownParent: $('#generateReceiptModal')
    });

    $('#statement_patient_id').select2({
        placeholder: 'Select Patient',
        allowClear: true,
        width: '100%',
        dropdownParent: $('#patientStatementModal')
    });
    ...
    $('#patient_id').on('change', function() {
        ...
        if (patientId) {
            transactionSelect.append('<option value="" disabled>Loading transactions...</option>');

            // Load patient transactions
            $.get(`{{ url('/api/patients') }}/${patientId}/transactions`)
                .done(function(response) {
                    var transactions = Array.isArray(response)
                        ? response
                    ...
                    if (!transactions.length) {
                        transactionSelect.append('<option value="" disabled>No transactions found for this patient</option>');
                        return;
                    }

                    transactions.forEach(function(transaction) {
                        var amount = parseFloat(transaction.amount || 0).toFixed(2);
                        var date = transaction.transaction_date
                            ? new Date(transaction.transaction_date).toLocaleDateString()
                            : '';
                        var label = transaction.description || transaction.transaction_number || 'Transaction';

                        transactionSelect.append(`
                            <option value="${transaction.id}">
                                ${label} - Tsh ${amount}${date ? ` (${date})` : ''}
                            </option>
                        `);
                    });
                })
                .fail(function(xhr) {
                    transactionSelect.empty().append('<option value="">Select Transaction</option>');
                    transactionSelect.append('<option value="" disabled>Failed to load transactions</option>');

                    var errorMessage = 'Unable to load patient transactions';
    ...
    $('#generateReceiptForm').on('submit', function(e) {
        ...
        if (deliveryMethod === 'email') {
            $.ajax({
                url: `/financial/receipts/${transactionId}/email`,
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}'
                },
                ...
                error: function(xhr) {
                    var errorMessage = 'Failed to send receipt via email';
                    if (xhr.responseJSON && xhr.responseJSON.message) {
                        errorMessage = xhr.responseJSON.message;
                    }
                    showAlert('error', errorMessage);
        ...
        if (deliveryMethod === 'print') {
            window.open(`/financial/receipts/${transactionId}/print?format=${encodeURIComponent(format)}`, '_blank');
        } else {
            window.open(`/financial/receipts/${transactionId}/generate?format=${encodeURIComponent(format)}`, '_blank');
        }
    ...
    $('#patientStatementForm').on('submit', function(e) {
        ...
        var submitBtn = $(this).find('button[type="submit"]');
        var patientId = $('#statement_patient_id').val();
        var dateFrom = $('#date_from').val();
        var dateTo = $('#date_to').val();
        var includeInsurance = $('#include_insurance').is(':checked');
        ...
        params.append('include_insurance', includeInsurance ? '1' : '0');

        var statementUrl = `{{ url('/financial/receipts/patient') }}/${patientId}/statement`;
    ...
    $('#dailySummaryForm').on('submit', function(e) {
        ...
        var summaryDate = $('#summary_date').val();
        var submitBtn = $(this).find('button[type="submit"]');
        ...
        var summaryUrl = `{{ url('/financial/receipts/daily-summary') }}?date=${encodeURIComponent(summaryDate)}`;
...
function viewReceipt(id) {
    window.open(`/financial/receipts/${id}/preview`, '_blank');
}

function downloadReceipt(id) {
    window.open(`/financial/receipts/${id}/generate?format=pdf`, '_blank');
}

function emailReceipt(id) {
    if (confirm('Send receipt via email to patient?')) {
        $.ajax({
            url: `/financial/receipts/${id}/email`,
            method: 'POST',
            data: {
                _token: '{{ csrf_token() }}'
            },
