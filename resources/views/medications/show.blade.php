        ...
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        <p class="form-control-plaintext mb-2">Drug Classes:</p>
                        @if($medication->drugClasses->isNotEmpty())
                            @php
                                $classesByCategory = $medication->drugClasses
                                    ->sortBy('name')
                                    ->groupBy(fn($dc) => $dc->category ?: 'Other');
                            @endphp
                            @foreach($classesByCategory as $category => $classes)
                                <div class="mb-2">
                                    <small class="text-muted d-block mb-1">{{ $category }}</small>
                                    @foreach($classes as $drugClass)
                                        <span class="badge bg-primary me-1 mb-1">{{ $drugClass->name }}</span>
                                    @endforeach
                                </div>
                            @endforeach
                        @else
                            <span class="text-muted">No drug classes assigned</span>
                        @endif
                    </div>
