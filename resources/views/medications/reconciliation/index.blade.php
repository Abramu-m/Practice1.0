                    ...
                    <div class="list-group list-group-flush" style="max-height: 400px; overflow-y: auto;">
                        ...
                        <div class="list-group-item border-0">
                            <div class="d-flex align-items-start">
                                <div class="flex-shrink-0">
                                    @php
                                        $cIcon = match($correction->correction_type) {
                                            'auto', 'expired_status' => 'bg-info',
                                            default => 'bg-success',
                                        };
                                    @endphp
                                    <span class="badge {{ $cIcon }} rounded-pill">
                                ...
                                <div class="flex-grow-1 ms-3">
                                    <div class="d-flex justify-content-between align-items-start">
                                        <div>
                                            <p class="mb-1 fw-medium">{{ $correction->medication->name ?? 'Unknown' }}</p>
                                            <p class="mb-1 text-muted small">
                                                {{ $correction->field_corrected }}:
                                                {{ $correction->old_value ?? '—' }} → {{ $correction->new_value ?? '—' }}
                                            </p>
                                            <small class="text-muted">
                                                {{ $correction->correction_date?->diffForHumans() }}
                                                @if($correction->correctedBy)
                                                    &middot; {{ $correction->correctedBy->name }}
                                                @else
                                                    &middot; System
                                                @endif
                                            </small>
                                        </div>
                                        <small class="text-muted">{{ $correction->location->name ?? 'All locations' }}</small>
                                    </div>
                                </div>
                            </div>
                        </div>
