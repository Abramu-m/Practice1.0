...
<head>
    <meta charset="utf-8">
    <title>Microbiology Monthly Report</title>
    <style>
        body {
...
<body>
    ...
    <table class="header-info" style="margin-bottom:4px">
        ...
        <tr>
            <td class="text-center">
                <strong style="font-size:12px">MICROBIOLOGY MONTHLY REPORT</strong>
            </td>
        </tr>
    </table>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="text-align:center; width:60px">POSITIVE</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($rows as $row)
            <tr class="{{ $row->is_bold ? 'section-header' : '' }}">
                <td>{{ $row->row_label }}</td>
                <td style="text-align:center">{{ $totals[$row->row_key] !== null ? $totals[$row->row_key] : '' }}</td>
                <td style="text-align:center">{{ $positives[$row->row_key] !== null ? $positives[$row->row_key] : '' }}</td>
            </tr>
            @endforeach
        </tbody>
        <tfoot>
