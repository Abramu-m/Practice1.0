...
<head>
    <meta charset="utf-8">
    <title>Parasitology Monthly Report</title>
    <style>
        body {
...
<body>
    ...
    <table class="header-info" style="margin-bottom:4px">
        ...
        <tr>
            <td class="text-center">
                <strong style="font-size:12px">PARASITOLOGY MONTHLY REPORT</strong>
            </td>
        </tr>
    </table>
    ...
    <table>
        ...
        <tbody>
            @foreach ($rows as $row)
                ...
                <tr>
                    <td>{{ $row->row_label }}</td>
                    <td style="text-align:center">{{ $totals[$row->row_key] ?? '' }}</td>
                    <td style="text-align:center">{{ $positives[$row->row_key] ?? '' }}</td>
                </tr>
                @endif
            @endforeach
        </tbody>
