...
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Edit Requisition - {{ $requisition->requisition_number }}</h5>
                    <div class="d-flex gap-2">
                        <a href="{{ route('store.requisitions.show', $requisition) }}" class="btn btn-secondary btn-sm">
                ...
                <div class="card-body">
                    <form action="{{ route('store.requisitions.update', $requisition) }}" method="POST" id="requisitionForm">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="requesting_location_id" id="requesting_location_id" class="form-control @error('requesting_location_id') is-invalid @enderror" required>
                                        ...
                                        @foreach($locations as $location)
                                            <option value="{{ $location->id }}" {{ (old('requesting_location_id') ?? $requisition->requesting_location_id) == $location->id ? 'selected' : '' }}>
                                                {{ $location->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="priority" id="priority" class="form-control @error('priority') is-invalid @enderror">
                                        ...
                                        <option value="emergency" {{ (old('priority') ?? $requisition->priority) == 'emergency' ? 'selected' : '' }}>Emergency</option>
                                    </select>
                                    @error('priority')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                        ...
                        <div class="row mb-4">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="purpose" id="purpose" class="form-control @error('purpose') is-invalid @enderror" rows="3" placeholder="Describe the purpose of this requisition...">{{ old('purpose') ?? $requisition->purpose }}</textarea>
                                    @error('purpose')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
        ...
        <!-- Items Table -->
        <div class="card mb-4" id="itemsTableCard" @if($requisition->items->count() > 0) style="display: block;" @else style="display: none;" @endif>
            <div class="card-header">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="d-flex justify-content-end gap-2">
                                    <a href="{{ route('store.requisitions.show', $requisition) }}" class="btn btn-secondary">Cancel</a>
                                    <button type="submit" name="action" value="draft" class="btn btn-outline-primary">Save as Draft</button>
                                    <button type="submit" name="action" value="submit" class="btn btn-primary">Update & Submit</button>
                                </div>
                            </div>
                        </div>
                    </form>
...
$(document).ready(function() {
    ...
    console.log('Existing items:', existingItems); // Debug line
    
    // Load existing items into the table
    existingItems.forEach(function(item, index) {
        console.log('Loading item:', item); // Debug line
    ...
    $('#addItemBtn').click(function() {
        ...
        const unitCost = medication.unit_cost || 0;
        const totalCost = indentQty * unitCost;
        ...
        const row = `
            <tr data-index="${itemIndex}">
                ...
                <td>
                    $${unitCost.toFixed(2)}
                    <input type="hidden" name="items[${itemIndex}][unit_cost]" value="${unitCost}">
                </td>
                <td class="total-cost">
                    $${totalCost.toFixed(2)}
                </td>
    ...
    // Function to add existing items to table
    function addExistingItemToTable(item) {
        console.log('Adding existing item to table:', item); // Debug line
        
        // Find the medication data
        const medication = medicationsData.find(med => med.id == item.item_id);
        
        console.log('Found medication:', medication); // Debug line
        
        if (medication) {
            console.log('Item unit_cost:', item.unit_cost, 'Medication unit_cost:', medication.unit_cost); // Debug
            const unitCost = parseFloat(item.unit_cost || medication.unit_cost || 0);
            const totalCost = parseFloat(item.requested_quantity) * unitCost;
            console.log('Calculated unitCost:', unitCost, 'totalCost:', totalCost); // Debug
            ...
            const row = `
                <tr data-index="${itemIndex}">
                    ...
                    <td>
                        ...
                        <input type="hidden" name="items[${itemIndex}][requested_quantity]" value="${item.requested_quantity}">
                    </td>
                    <td>
                        $${unitCost.toFixed(2)}
                    ...
                    <td>
                        ${item.justification || '-'}
                        <input type="hidden" name="items[${itemIndex}][justification]" value="${item.justification || ''}">
                    </td>
                    <td>
                        <button type="button" class="btn btn-danger btn-sm remove-item-btn" data-index="${itemIndex}">
                            <i class="fas fa-trash"></i>
            ...
            console.log('Generated row HTML:', row); // Debug line
            $('#itemsContainer').append(row);
            itemIndex++;
        } else {
            console.error('Medication not found for item:', item); // Debug line
        }
    }
