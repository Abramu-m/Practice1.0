        ...
        <div class="col-md-8 offset-md-2">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">
                        <i class="fas fa-edit"></i> Edit Consultation Fee
                    </h3>
                    <div class="card-tools">
                        <a href="{{ route('consultation_fees.show', $consultationFee->id) }}" class="btn btn-info btn-sm">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('consultation_fees.update', $consultationFee->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                                    ...
                                    <select class="form-control @error('doctor_id') is-invalid @enderror" id="doctor_id" name="doctor_id" required>
                                        ...
                                        @foreach($doctors as $doctor)
                                            <option value="{{ $doctor->doctor_id }}" 
                                                {{ old('doctor_id', $consultationFee->doctor_id) == $doctor->doctor_id ? 'selected' : '' }}>
                                    ...
                                    <select class="form-control @error('patient_category_id') is-invalid @enderror" id="patient_category_id" name="patient_category_id" required>
                                        ...
                                        @foreach($patientCategories as $category)
                                            <option value="{{ $category->id }}" 
                                                {{ old('patient_category_id', $consultationFee->patient_category_id) == $category->id ? 'selected' : '' }}>
                                    ...
                                    <select class="form-control @error('visit_type_id') is-invalid @enderror" id="visit_type_id" name="visit_type_id" required>
                                        ...
                                        @foreach($visitTypes as $visitType)
                                            <option value="{{ $visitType->id }}" 
                                                {{ old('visit_type_id', $consultationFee->visit_type_id) == $visitType->id ? 'selected' : '' }}>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" step="0.01" min="0" max="999999.99" 
                                           ...
                                           value="{{ old('cash_amount', $consultationFee->cash_amount) }}" 
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="covered_amount">Covered Amount (Tsh) <span class="text-danger">*</span></label>
                                    <input type="text" step="0.01" min="0" max="999999.99" 
                                           class="form-control @error('covered_amount') is-invalid @enderror" 
                                           id="covered_amount" name="covered_amount" 
                                           value="{{ old('covered_amount', $consultationFee->covered_amount) }}" 
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      ...
                                      placeholder="Optional description for this fee structure">{{ old('description', $consultationFee->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('status') is-invalid @enderror" id="status" name="status" required>
                                <option value="1" {{ old('status', $consultationFee->status) == 1 ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('status', $consultationFee->status) == 0 ? 'selected' : '' }}>Inactive</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3 mt-4">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> Update Consultation Fee
                            </button>
                            <a href="{{ route('consultation_fees.show', $consultationFee->id) }}" class="btn btn-info">
                                <i class="fas fa-eye"></i> View Details
                            </a>
...
$(document).ready(function() {
    var originalDoctorId = {{ $consultationFee->doctor_id }};
    var originalCategoryId = {{ $consultationFee->patient_category_id }};
    var originalVisitTypeId = {{ $consultationFee->visit_type_id }};
    ...
    $('#doctor_id, #patient_category_id, #visit_type_id').change(function() {
        ...
        if (doctorId && categoryId && visitTypeId) {
            // Don't check if it's the same combination as the original
            if (doctorId == originalDoctorId && categoryId == originalCategoryId && visitTypeId == originalVisitTypeId) {
                return;
            }
            ...
            .done(function(data) {
                if (data.fee !== null) {
                    alert('Warning: A fee structure already exists for this combination. Fee: $' + data.fee);
                }
            });
        }
    });
